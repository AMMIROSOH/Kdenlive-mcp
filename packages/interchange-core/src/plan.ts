...
export interface ImportChange {
  readonly kind: 'replace-project' | 'create-project';
...
export function buildImportPlan(input: {
  ...
  readonly parsed: InterchangeParse;
}): ImportPlan {
  const identityMatches =
    input.current === undefined ||
    input.parsed.provenance.projectId === input.current.id;
  ...
  return {
    ...
    changes: [
      {
        kind:
          input.mode === 'new-project' ? 'create-project' : 'replace-project',
        summary:
          input.mode === 'new-project'
            ? 'Create a canonical project from the imported interchange.'
            : 'Apply the reviewed external project state as one canonical revision.',
      },
    ],
    fidelity: identityMatches
      ? input.parsed.fidelity
      : [
          ...input.parsed.fidelity,
          {
            code: 'INTERCHANGE_IDENTITY_MISMATCH',
            severity: 'error',
            message: 'The source was not exported from this canonical project.',
          },
        ],
    canApply,
  };
