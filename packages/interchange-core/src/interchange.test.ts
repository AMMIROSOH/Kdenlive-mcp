...
describe('interchange core', () => {
  it('exports deterministic, provenance-preserving Kdenlive and OTIO documents', () => {
    ...
    expect(exportInterchange(project, 'kdenlive').contents).toBe(kdenlive.contents);
    expect(parseInterchange(kdenlive.contents, 'kdenlive').project).toEqual(project);
  ...
  it('rejects XML entity declarations and foreign round-trips', () => {
    expect(() => parseInterchange('<!DOCTYPE x [<!ENTITY x "boom">]><mlt/>', 'kdenlive')).toThrow(/DTD/u);
    const current = createProject('Current');
    const foreign = parseInterchange(exportInterchange(createProject('Foreign'), 'otio').contents, 'otio');
    expect(buildImportPlan({ mode: 'roundtrip', current, parsed: foreign }).canApply).toBe(false);
  });
});
