...
const definitions: readonly [string, string, readonly string[]][] = [
  ...
  ['silence-filler', 'Silence and filler review', ['timeline_diagnostics_submit']],
  ...
  ['youtube-long-form', 'YouTube long-form delivery', ['marker_edit', 'render_submit']],
  ['shorts', 'Short-form delivery', ['project_get', 'render_submit']],
  ['chapters', 'Chapter markers', ['marker_edit']],
  ['delivery', 'Final delivery', ['timeline_diagnostics_submit', 'render_submit', 'job_verify']],
];

export const EDITING_RECIPES: readonly EditingRecipe[] = definitions.map(([id, title, prerequisites]) => ({
  id, version: 1, title, prerequisites,
  safetyNotes: ['Create a checkpoint before destructive edits.', 'Inspect previews and diagnostics before export.', 'Apply mutations with the current expectedRevision.'],
  steps: ['Inspect the current timeline.', 'Run the listed MCP tools in sequence.', 'Review the resulting timeline and diagnostics.', 'Render only after the review is satisfactory.'],
}));
